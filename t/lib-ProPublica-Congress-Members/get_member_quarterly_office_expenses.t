...
HAPPY_PATH: {
    ...
    my $expenses = $members_obj->get_member_quarterly_office_expenses(
...
EXCEPTIONS: {
    ...
    note( 'member values' );
    dies_ok { $members_obj->get_member_quarterly_office_expenses( year => 2009, quarter => 4 ) }
    ...
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => '', year => 2009, quarter => 4 ) }
              'dies if member_id argument is empty string';
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => '_ABC123', year => 2009, quarter => 4 ) }
    ...
    note( 'year values' );
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', quarter => 4 ) }
              'dies if year argument is missing';
    like $@, qr/The year argument is required/,
         'exception indicates year argument is required';
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', year => 2008, quarter => 4 ) }
              'dies if year argument is < 2009';
    like $@, qr/The year argument must be a >= 2009/,
         'exception indicates year argument must be >= 2009';
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', year => 'a', quarter => 4 ) }
              'dies if year argument is a';

    note( 'quarter values' );
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', year => 2009 ) }
    ...
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', year => 2009, quarter => 5 ) }
              'dies if quarter argument is 5';
    like $@, qr/The quarter argument must be 1-4/,
         'exception indicates quarter argument must be 1-4';
    dies_ok { $members_obj->get_member_quarterly_office_expenses( member_id => 'ABC123', year => 2009, quarter => 'a' ) }
