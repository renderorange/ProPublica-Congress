...
if ( $opts{help} || !$opts{congress} || !$opts{chamber} ) {
    print "Usage: get_members_leaving_office.pl --congress 111 --chamber house\n";
    exit;
}
...
my $members = $members_obj->get_members_leaving_office( congress => $opts{congress}, chamber => $opts{chamber} );
