...
if ( $opts{help} || !$opts{member_id_1} || !$opts{member_id_2} || !$opts{congress} || !$opts{chamber} ) {
    print "Usage: compare_member_vote_positions.pl --member_id_1 Y000031 --member_id_2 Z000004 --congress 102 --chamber house\n" . 
...
my $comparison = $members_obj->compare_member_vote_positions( member_id_1 => $opts{member_id_1}, member_id_2 => $opts{member_id_2}, congress => $opts{congress}, chamber => $opts{chamber} );
